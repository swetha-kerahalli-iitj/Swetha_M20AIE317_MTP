...
def train(epoch, model, optimizer, args, use_cuda=False, verbose=True, mode='encoder'):
    ...
    for batch_idx in range(int(args.num_block / args.batch_size)):
        ...
        if mode == 'encoder':
            fwd_noise = generate_noise(noise_shape, args, snr_low=args.train_enc_channel_low,
                                       snr_high=args.train_enc_channel_high, mode='encoder')
        else:
            fwd_noise = generate_noise(noise_shape, args, snr_low=args.train_dec_channel_low,
                                       snr_high=args.train_dec_channel_high, mode='decoder')
