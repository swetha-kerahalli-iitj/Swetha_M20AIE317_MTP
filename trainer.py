    ...
    for epoch in range(start_epoch, args.num_epoch + 1):
        ...
        if args.num_train_enc > 0:
             for idx in range(args.num_train_enc):
                train(epoch, model, enc_optimizer, args,SNR,blocklen,coderate_k,coderate_n,mod_type, use_cuda=use_cuda, mode='encoder')

        if args.num_train_dec > 0:
           for idx in range(args.num_train_dec):
               train(epoch, model, dec_optimizer, args,SNR,blocklen,coderate_k,coderate_n,mod_type,use_cuda=use_cuda, mode='decoder')

        this_loss, this_ber, this_bler = validate(model, general_optimizer, args,SNR,blocklen,coderate_k,coderate_n,mod_type ,use_cuda=use_cuda)
...
def train(epoch, model, optimizer, args,SNR,block_len=10,coderate_k=1,coderate_n=3,mod_type='QAM16', use_cuda = False, verbose = True, mode = 'encoder'):
    ...
    if verbose:
        print('====> Epoch: {} with snr {} Average loss AWGN: {:.8f} loss Rayleigh: {:.8f} loss Rician: {:.8f} '.format(epoch, SNR,train_loss_awgn,train_loss_ray,train_loss_rici), \
...
def validate(model, optimizer, args, SNR,block_len=10, coderate_k=1, coderate_n=3,mod_type="QAM16", use_cuda = False, verbose = True,
            ):
    ...
    if verbose:
        print('====> Test set BCE loss with SNR {} for AWGN'.format(SNR), float(test_bce_loss_awgn),
              'Custom Loss',float(test_custom_loss_awgn),
              'with ber ', float(test_ber_awgn),
              'with bler ', float(test_bler_awgn),
        )
        print('====> Test set BCE loss with SNR {} for Rayleigh'.format(SNR), float(test_bce_loss_ray),
              'Custom Loss', float(test_custom_loss_ray),
              'with ber ', float(test_ber_ray),
              'with bler ', float(test_bler_ray),
              )
        print('====> Test set BCE loss with SNR {} for Rician'.format(SNR), float(test_bce_loss_rici),
