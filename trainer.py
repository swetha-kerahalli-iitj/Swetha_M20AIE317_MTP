...
def train(epoch, model, optimizer, args,channel='awgn',blocklen =10,coderate_k=1,coderate_n=3 ,use_cuda = False, verbose = True, mode = 'encoder'):
    ...
    for batch_idx in range(int(args.num_block/args.batch_size)):
        ...
        noise_shape = (args.batch_size, blocklen, coderate_n)
        ...
        output, code = model(X_train, fwd_noise,channel)
...
def validate(model, optimizer, args,channel='awgn',blocklen =10,coderate_k=1,coderate_n=3 , use_cuda = False, verbose = True):
    ...
    with torch.no_grad():
        ...
        for batch_idx in range(num_test_batch):
            ...
            optimizer.zero_grad()
            output, codes = model(X_test, fwd_noise,channel)
...
def test(model, args,filename ,channel='awgn',blocklen =10,coderate_k=1,coderate_n=3 , block_len = 'default',use_cuda = False):
    ...
    for sigma, this_snr in zip(sigmas, snrs):
        ...
        with torch.no_grad():
            ...
            for batch_idx in range(num_test_batch):
                ...
                X_hat_test, the_codes = model(X_test, fwd_noise,channel)
