...
class ENC_turbofy_rate2(ENCBase):
    def __init__(self, args, p_array,coderate_k,coderate_n,blocklen):
...
class ENC_interCNN(ENCBase):
    def __init__(self, args, p_array,coderate_k,coderate_n,blocklen):
        ...
        if self.args.encoder == 'TurboAE_rate3_cnn':
            self.enc_cnn_1       = SameShapeConv1d(num_layer=args.enc_num_layer, in_channels=self.coderate_k,
                                                      out_channels= args.enc_num_unit, kernel_size = args.enc_kernel_size)

            self.enc_cnn_2       = SameShapeConv1d(num_layer=args.enc_num_layer, in_channels=self.coderate_k,
                                                      out_channels= args.enc_num_unit, kernel_size = args.enc_kernel_size)

            self.enc_cnn_3       = SameShapeConv1d(num_layer=args.enc_num_layer, in_channels=self.coderate_k,
                                                      out_channels= args.enc_num_unit, kernel_size = args.enc_kernel_size)
        else: # Dense
            self.enc_cnn_1       = DenseSameShapeConv1d(num_layer=args.enc_num_layer, in_channels=self.coderate_k,
                                                      out_channels= args.enc_num_unit, kernel_size = args.enc_kernel_size)

            self.enc_cnn_2       = DenseSameShapeConv1d(num_layer=args.enc_num_layer, in_channels=self.coderate_k,
                                                      out_channels= args.enc_num_unit, kernel_size = args.enc_kernel_size)

            self.enc_cnn_3       = DenseSameShapeConv1d(num_layer=args.enc_num_layer, in_channels=self.coderate_k,
                                                      out_channels= args.enc_num_unit, kernel_size = args.enc_kernel_size)
...
class ENC_interCNN2Int(ENCBase):
    def __init__(self, args, p_array1, p_array2,coderate_k,coderate_n,blocklen):
        # turbofy only for code rate 1/3
...
class ENC_interCNN2D(ENCBase):
    def __init__(self, args, p_array,coderate_k,coderate_n,blocklen):
        ...
        self.enc_cnn_1       = CNN2d(num_layer=args.enc_num_layer, in_channels=self.coderate_k,
                                                  out_channels= args.enc_num_unit, kernel_size = args.enc_kernel_size)

        self.enc_linear_1    =  CNN2d(num_layer=1, in_channels= args.enc_num_unit,
                                      out_channels= 1, kernel_size = 1, no_act=True)

        self.enc_cnn_2       = CNN2d(num_layer=args.enc_num_layer, in_channels=self.coderate_k,
                                                  out_channels= args.enc_num_unit, kernel_size = args.enc_kernel_size)

        self.enc_linear_2    = CNN2d(num_layer=1, in_channels= args.enc_num_unit,
                                                  out_channels= 1, kernel_size = 1, no_act=True)

        self.enc_cnn_3       = CNN2d(num_layer=args.enc_num_layer, in_channels=self.coderate_k,
                                                  out_channels= args.enc_num_unit, kernel_size = args.enc_kernel_size)
    ...
    def forward(self, inputs):
        img_size   = int(np.sqrt(self.blocklen))
        ...
        x_tx = x_tx.view(self.args.batch_size, self.code_rate_n, self.blocklen)
...
def turbo_enc(X_train_raw, args, p_array,coderate_k,coderate_n,blocklen):
    ...
    interleaver = RandInterlv.RandInterlv(blocklen, 0)
