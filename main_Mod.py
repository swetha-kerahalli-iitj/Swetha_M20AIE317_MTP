...
from trainer import train_model_mod
...
if __name__ == '__main__':
    ...
    for blocklen in (args.block_len):
        # for mod_type,mod_rate in zip( args.modtype, args.mod_rate):
        mod_rate = 2
        mod_type = 'QAM2'
        coderate_n = args.code_rate_n
        coderate_k = args.code_rate_k
        for channel in (args.channel):
            ...
            print('Training Model for :',
                  ...
                  'channel =>',channel,
                  'mod type=>',mod_type)
            prefix = 'bl_' + str(blocklen) + '_' + '_k_' + str(coderate_k) +  '_n_' + str(coderate_n)+'_chl_'+channel+'_mod_QAM' + str(mod_rate)
            ...
            train_model_mod(args,timestamp,store_files,start_epoch,use_cuda,blocklen,coderate_k,coderate_n,channel,mod_rate=mod_rate)
            ...
            data_file_all.write(
                str(blocklen) +' ' + str(coderate_k) + ' '+ str(coderate_n) + ' '+ str(float(coderate_k/coderate_n)) +' '+ channel +' '+ mod_type +' '+str(filename) + ' ' + str(test_filename) + "\n")
