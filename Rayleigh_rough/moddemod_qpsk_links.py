...
class LinkModel:
    ...
    def link_performance_full_metrics_iteration(self, SNR,bit_err,send_chunk, receive_size,code_rate: Fraction = Fraction(1, 1)):
        # Propagate some bits
        ...
        for id_SNR in range(len(SNRs)):
            ...
            for id_tx in range(tx_max):
                if stop_on_surpass_error and bit_err.sum() > err_min:
                    break
                bit_err[id_tx],chunk_loss[id_tx],number_chunks_per_send[id_tx] = \
                    self.link_performance_full_metrics_iteration(SNRs, err_min, send_chunk, code_rate,
