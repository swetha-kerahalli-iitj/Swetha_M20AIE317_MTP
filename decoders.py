...
class DEC_LargeRNN(torch.nn.Module):
    ...
    def forward(self, received):
        ...
        # Turbo Decoder
        r_sys     = received[:,:,0].view((self.args.batch_size, self.blocklen, 1))
        r_sys_int = self.interleaver(r_sys)
        r_par1    = received[:,:,1].view((self.args.batch_size, self.blocklen, 1))
        r_par2    = received[:,:,2].view((self.args.batch_size, self.blocklen, 1))

        #num_iteration,
        prior = torch.zeros((self.args.batch_size, self.blocklen, self.args.num_iter_ft)).to(self.this_device)
...
class DEC_LargeCNN(torch.nn.Module):
    ...
    def forward(self, received):
        ...
        if self.args.is_variable_block_len:
            self.blocklen = received.shape[1]
            # reset interleaver
            if self.args.is_interleave != 0:           # fixed interleaver.
                ...
                p_array = rand_gen.permutation(arange(self.blocklen))
                self.set_interleaver(p_array)
        else:
            self.blocklen = self.blocklen
...
class DEC_LargeCNN2D(torch.nn.Module):
    ...
    def forward(self, received):
        received = received.type(torch.FloatTensor).to(self.this_device)
        received = received.view(self.args.batch_size, self.args.img_size, self.args.img_size, self.coderate_n)
        ...
        final      = torch.sigmoid(self.deinterleaver(x_plr))
        final      = final.view(self.args.batch_size, self.args.code_rate_k, self.blocklen)
...
class DEC_LargeRNN_rate2(torch.nn.Module):
    ...
    def forward(self, received):
        ...
        # Turbo Decoder
        r_sys     = received[:,:,0].view((self.args.batch_size, self.blocklen, 1))
        r_int     = received[:,:,1].view((self.args.batch_size, self.blocklen, 1))
...
class DEC_LargeCNN_rate2(torch.nn.Module):
    ...
    def forward(self, received):
        ...
        r_sys       = received[:,:,0].view((self.args.batch_size, self.blocklen, 1))
        r_sys_int   = self.interleaver(r_sys)
        r_par       = received[:,:,1].view((self.args.batch_size, self.blocklen, 1))
...
class NeuralTurbofyDec(torch.nn.Module):
    def __init__(self, args, p_array):
        ...
        self.dec_rnn  = torch.nn.GRU(self.coderate_n + args.num_iter_ft - 1 , args.dec_num_unit, num_layers=2, bias=True, batch_first=True,
                                   dropout=args.dropout, bidirectional=True)
    ...
    def forward(self, inputs):
        ...
        # Turbo Decoder
        r_sys     = inputs[:,:,0].view((input_shape[0], self.blocklen, 1))
        r_sys_int = self.interleaver(r_sys)
        r_par1    = inputs[:,:,1].view((input_shape[0], self.blocklen, 1))
        r_par2    = inputs[:,:,2].view((input_shape[0], self.blocklen, 1))

        #num_iteration,
        prior = torch.zeros((input_shape[0], self.blocklen, self.args.num_iter_ft)).to(self.device)
