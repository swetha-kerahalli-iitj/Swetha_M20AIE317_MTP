...
class Modulation(torch.nn.Module):
    def __init__(self, args,blocklen,coderate_k,coderate_n,mod_rate):
        ...
        self.mod_layer = SameShapeConv1d(num_layer=args.mod_num_layer, in_channels=self.mod_rate,
    ...
    def forward(self, inputs):
        ...
        inputs_flatten = inputs.view(self.args.batch_size, int(self.blocklen * self.coderate_n / self.mod_rate), self.mod_rate)
...
class DeModulation(torch.nn.Module):
    def __init__(self, args,blocklen,coderate_k,coderate_n,mod_rate):
        ...
        self.demod_final = SameShapeConv1d(num_layer=1, in_channels=args.demod_num_unit,
                                          out_channels= self.mod_rate, kernel_size = 1,  no_act = True)
    ...
    def forward(self, inputs):
        ...
        demod_codes = demod_symbols.reshape(self.args.batch_size, self.blocklen, self.coderate_n)
